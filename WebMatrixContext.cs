...
namespace DesignFactory.WebMatrix.Executer
{
    ...
    {
        public static void Initialize(string extensionName, IWebMatrixHost webMatrixHost, IEditorTaskPanelService editorTaskPanelService)
        {
            if (webMatrixHost == null || editorTaskPanelService == null)
            {
                MessageBox.Show(String.Format("Extension '{0}' not working correctly. Extension must call DesignFactory.WebMatrix.Executer.Initialize() function correctly. WebMatrixHost or EditorTaskPanelService is null.", extensionName), "Extension failure", MessageBoxButton.OK);
            }
            else
            {
                if (!_isInitialised)
       ...
       public static void InitializeTabs(object sender, EventArgs eventArgs)
        {
